...
public class Validation
{
    ...
    internal bool checkIfProdectExistsInProductMaster(string Product_id)
    {
        DbCon = new DatabaseConnection();
        conString = DbCon.GetConString();
        DbCon.connection_String = conString;
        DbCon.Sql = "select * from Product_Master with (nolock) where product_id='"+Product_id+"'";
        ds = DbCon.GetConnection;
        MaxRows = ds.Tables[0].Rows.Count;
    ...
    internal bool CheckIfGTINExistsInXref(string GTIN)
    {
        DbCon = new DatabaseConnection();
        conString = DbCon.GetConString();
        DbCon.connection_String = conString;
        DbCon.Sql = "select * from gtin_xref with (nolock) where gtin='" + GTIN + "'";
        ds = DbCon.GetConnection;
        MaxRows = ds.Tables[0].Rows.Count;
