    ...
    public class Order
    {
        ...
        *I.E. if a order has 2 large red and two medium red then we reeturn true
        ...
        public bool SplitProduct(int CurrentProduct, CartonInfo[] Boxes)
        {
            ...
            Log.Write("calling SplitProduct for " + Pieces[CurrentProduct].ProductID);
            ...
            if (Pieces[CurrentProduct].VolumeOfAPiece <= 0)
            {
                return false;
            }

            MaxPiecesPerCarton = Convert.ToInt32(Boxes[0].GetMaxVolume() / Pieces[CurrentProduct].VolumeOfAPiece);
            MaxPiecesPerCartonVolume = MaxPiecesPerCarton / Pieces[CurrentProduct].VolumeOfAPiece;
            if (MaxPiecesPerCarton==0)
            {
                MaxPiecesPerCarton = 1;
                Log.Write(CurrentProduct + ": tried to divide by 0");
            }
                ...
                if (OBSD[i].UnitOfMesure == "EA")
                {
                    if (OBSD[i].AllocationStatus == "N")
                    {
                        ...
                        else
                        {
                            ...
                            InventoryTable[] InvTable = new InventoryTable[400];
                            InventoryAllocationTabel[] InvAllTable = new InventoryAllocationTabel[400];
        ...
        private string CreateCasePick(OutboundShipmentDetails OBSD, string ProductID, string PiecePickLocation, int NumCasesRequired, int[] CaseIndexCount, string CarrierID, int? QuantityToAllocate)
        {
           ...
           INVENTORY I with(NOLOCK), LOCATIONS L with(NOLOCK),LOOKUP_VALUES LV with(NOLOCK) 
            ...
            var OrderPrimary = "Order By Caseqty desc, I.LOCATION_ID asc, LV.DESCRIPTION";
        ...
        {
            if (invAvail.FromLocation.Length == 7 &&( invAvail.FromLocation.Substring(0, 2) != "BW" || invAvail.FromLocation.Substring(0, 2)!="SW"))
            {
            ...
            int QuantityToAllocate, string carrierID)
        {
            ...
            InventoryTable[] InvTable = new InventoryTable[9999];
            int InvTableCount = 0;
            int MoveQCount = 0;
