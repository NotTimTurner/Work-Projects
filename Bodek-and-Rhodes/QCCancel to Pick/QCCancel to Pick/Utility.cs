...
public class Utility
{
    ...
    internal string GetVlevel(string p)
    {
        string UOM = p.Substring(3, 1);
        return UOM;
    }



    internal void updateVLEVEL(string VLEVEL, string LocationID)
    {
        DbCon = new DatabaseConnection();
        conString = DbCon.GetConString();
        DbCon.connection_String = conString;
        DbCon.Sql = "Update Locations with (rowlock) Set VLEVEL='" + VLEVEL+ "', _LAST_USER_ = 'TTURNER', _LAST_UPDATED_ = GetDate(), _LAST_MODULE_ = 'UpdateLG' Where location_id='"+LocationID  + "'";
        ds = DbCon.GetConnection;
    }

    internal int GetNumberOfCartons(string shipmentID)
    {
        DbCon = new DatabaseConnection();
        conString = DbCon.GetConString();
        DbCon.connection_String = conString;
        DbCon.Sql = "select shipment_id from outbound_shipment_details where shipment_id='"+shipmentID+"'";
        ds = DbCon.GetConnection;
    ...
    internal string GetPurchaseOrder(string shipmentID)
    {
        string PO = "";
         DatabaseConnection  DbCon = new DatabaseConnection();
            SqlConnection Con = new SqlConnection(DbCon.GetConString());
            SqlDataReader MyReader = null;
            SqlCommand SqlCommand = new SqlCommand("select Purchase_order from outbound_shipments where shipment_id='"+shipmentID+"'", Con);
            Con.Open();
            MyReader = SqlCommand.ExecuteReader();
            while (MyReader.Read())
            {
                PO = MyReader["Purchase_order"].ToString();
            }
            return PO;
    }


    internal void RemoveHold(string RID)
    {
        DbCon = new DatabaseConnection();
        conString = DbCon.GetConString();
        DbCon.connection_String = conString;
        DbCon.Sql = "update inventory set reservation_type = null where _rid_='"+RID+"'";
        ds = DbCon.GetConnection;
    }

    internal void DeleteMoveQueue(string RID)
    {
        DbCon = new DatabaseConnection();
        conString = DbCon.GetConString();
        DbCon.connection_String = conString;
        DbCon.Sql = "delete from move_queue where INV_rid='" + RID + "' and priority='30'";
        ds = DbCon.GetConnection;
    }

    internal double GetFullWMSQuantity(string ProductID)
    {
        double QTY = 0;
        DatabaseConnection DbCon = new DatabaseConnection();
        SqlConnection Con = new SqlConnection(DbCon.GetConString());
        SqlDataReader MyReader = null;
        SqlCommand SqlCommand = new SqlCommand("select Quantity from inventory where Product_id='" + ProductID + "' and location_id<> 'QCEMPTY'", Con);
        Con.Open();
        MyReader = SqlCommand.ExecuteReader();
        while (MyReader.Read())
        {
            QTY +=Convert.ToDouble( MyReader["Quantity"].ToString());
        }
        Con.Close();
        return QTY;
    }

    internal string GetPiecePickLocationForProductID(string Product_ID)
    {
        string PO = "";
        DatabaseConnection DbCon = new DatabaseConnection();
        SqlConnection Con = new SqlConnection(DbCon.GetConString());
        SqlDataReader MyReader = null;
        SqlCommand SqlCommand = new SqlCommand("select Piece_Pick_Location from product_master where product_id='" + Product_ID + "'", Con);
        Con.Open();
        MyReader = SqlCommand.ExecuteReader();
        while (MyReader.Read())
        {
            PO = MyReader["Piece_Pick_Location"].ToString();
        }
        Con.Close();
        return PO;
    }

    internal void DeleteMoveQueueByInv_RID(string RID,string Priority,string Type)
    {
        DbCon = new DatabaseConnection();
        conString = DbCon.GetConString();
        DbCon.connection_String = conString;
        DbCon.Sql = "delete from move_queue where INV_rid='" + RID + "' and priority='"+Priority+"' and type='"+Type+"'";
        ds = DbCon.GetConnection;
    }

    internal void UpdateInventoryLocationID(string RID, string PiecePick)
    {
        DbCon = new DatabaseConnection();
        conString = DbCon.GetConString();
        DbCon.connection_String = conString;
        DbCon.Sql = "update Inventory set Location_ID='"+PiecePick+"', _LAST_USER_ = 'TTURNER', _LAST_UPDATED_ = GetDate(), _LAST_MODULE_ = 'MassPutaway' where _RID_='"+RID+"'";
        ds = DbCon.GetConnection;
    }
}