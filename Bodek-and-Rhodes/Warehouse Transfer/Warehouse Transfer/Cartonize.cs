        ...
        public Order[] GetOrders(string OrderType)
        {
            try
            {
                ...
                using (WMSEntities wms = new WMSEntities())
                {
                    ...
                    var cancelOrder = from f in wms.ORDER_CANCEL_REQUESTS select f.EIS_ORDER_ID;

                    IQueryable<HOST_ORDERS> data;
                    CheckTruck();


                    //add in the cust ID for the new transfer here TIm
                    data = from D in wms.HOST_ORDERS where D.STATUS == "N" &&  !cancelOrder.Contains(D.EIS_ORDER_ID) && (D.TYPE == "WHT"||D.CUSTOMER_ID =="101210") orderby D.EIS_ORDER_ID select D;
                    orderCnt = (from D in wms.HOST_ORDERS where D.STATUS == "N" && !cancelOrder.Contains(D.EIS_ORDER_ID) && (D.TYPE == "WHT" || D.CUSTOMER_ID == "101210") select D).Count();
        ...
        internal bool RunProgram()
        {
            ...
            while (count < 3)
            {
                threads = GetThreads();
                    ...
                    {
                        Log.Write("Cartonize is Running, trying again in a minute "+(3-count)+" tries left");
                        Thread.Sleep(60000);
